...
class DoSchedulePage(webapp2.RequestHandler):
    ...
    def get(self):
        ...
        if schedules:
            for schedule in schedules:
                ...
                if self.NeedsNewJob(schedule, jobs):
                    ...
                    if target_host:
                        ...
                        # assume device build
                        (test_builds, device_builds,
                         gsi_builds) = build_list.ReadBuildInfo()
                        new_job.build_id = ""
                        if device_builds:
                            for device_build in device_builds:
                                self.LogPrintln(
                                    "-- check device_build %s (%s)" % (
                                        device_build.manifest_branch,
                                        device_build.manifest_branch))
                                ...
                                if (device_build.manifest_branch ==
                                        new_job.manifest_branch and
                                            new_job.build_target[0] ==
                                                candidate_device):
                                    if (not new_job.build_id or
                                        StrGT(device_build.build_id,
                                              new_job.build_id)):
                                        new_job.build_id = device_build.build_id
                        new_job.status = "LEASED"
                        new_job.timestamp = datetime.datetime.now()
                        new_job.put()
                        self.LogPrintln("** NEW LEASE")

        self.response.write("<br>".join(self.log_message))
    ...
    def NeedsNewJob(self, schedule, jobs):
        ...
        if not jobs:
            return False
        for job in jobs:
            if (job.manifest_branch == schedule.manifest_branch and
    ...
    def SelectTargetLab(self, schedule, labs, devices):
        ...
        Returns:
            hostname, a list of selected devices
        """
        ...
        self.LogPrintln("- Seeking lab and product %s %s" % (
